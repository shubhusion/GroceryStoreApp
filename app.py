...
from Database import User, Section, Product, Cart, Order, OrderProduct
import datetime
...
@app.route("/admin/section/create", methods=["GET", "POST"])  # route for Creating New Category
@login_required
def add_section():
    if request.method == "POST":
        # Retrieve form data
...
@app.route("/admin/section/update/<int:id>", methods=["GET", "POST"])  # route for updating existing Category
@login_required
...
@app.route("/admin/section/delete/<int:id>", methods=["GET", "POST"])  # route for deleting New Category
@login_required
...
@app.route("/admin/product/create", methods=["GET", "POST"])  # route for Creating New Product
@login_required
def add_product():
    if request.method == "POST":
        # Retrieve form data
...
@app.route("/admin/product/update/<int:id>", methods=["GET", "POST"])  # route for Creating New Product
@login_required
...
@app.route("/user/cart/view")  # Shows the items currently added to the user's shopping cart.
@login_required
def view_cart():
    return render_template(
        "view_cart.html",
...
@app.route("/user/cart/add/<int:id>")  # Adds the product with ID 123 to the user's cart.
@login_required
def addtocart(id):
...
@app.route("/user/cart/remove/<int:id>")  # Removes the product with ID 123 from the user's cart.
@login_required
...
@app.route("/user/cart/incr/<int:id>")  # increases the quantity of Product with the Specified Product id
def incrincart(id):
    prd = Product.query.get(id)
    if not prd:
...
@app.route("/user/cart/decr/<int:id>")  # decreases the quantity of Product with the Specified Product id
@login_required
def decrincart(id):
    prd = Product.query.get(id)
    if not prd:
...
def Orderprds():
    return render_template(
        "orders.html",
        orders=Order.query.filter_by(user_id=current_user.username),section=Section.query.all())
...
def api_register():
    if request.method == "POST":
        try:
            ...
            new_user = User(username=uname, name=name, password=password, email=email, admin=False)
...
class productAPI(Resource):
    ...
    def put(self, id):
        try:
            ...
            if name is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT001", error_message="Product Name is required")
            if unit is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT002", error_message="Product Unit is required")
            if price is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT003", error_message="Product Price is required")
            if quantity is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT004", error_message="Product quantity is required")
            if section_id is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT004", error_message="Product section_id is required")
    ...
    def post(self):
        # try:
            ...
            if quantity is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT004", error_message="Product quantity is required")
            if section_id is None:
                raise BuisnessValidationError(status_code=400, error_code="PRODUCT005", error_message="Product section_id is required")
            
            new_product = Product(name=name,manufacture_date=manufacture_date,expiry_date=expiry_date,unit=unit,price=price,quantity=quantity,section_id=section_id)
